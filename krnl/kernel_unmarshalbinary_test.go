...
func TestKernel_UnmarshalBinary_success(t *testing.T) {
...
func TestKernel_UnmarshalBinary_failure1024(t *testing.T) {

	for testNumber:=0; testNumber<20; testNumber++ {

		var p []byte
		{
			length := 1024 + 1 + testNumber

			p = make([]byte, length)

			for i:=0; i<len(p); i++ {
				p[i] = byte(rand.Int() % 256)
			}
		...
		if nil == err {
			t.Errorf("For test #%d, expected marshaling-binary to return an error — because length of []byte (%d) is too bit — but actually didn't." , testNumber, len(p))
			t.Logf("ERROR: %#v", err)
			continue
		}
	}
